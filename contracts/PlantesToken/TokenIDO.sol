...
contract KBKGovIDO is Ownable, IKBKGovIDO {
    using SafeERC20 for IERC20;

    uint internal constant PriSaleLimit = 2 * 1e8 * 10 ** 18;
    uint internal constant Day1 = 1 * 24 * 60 * 60;// 1 day
    uint internal constant Day7 = 7 * 24 * 60 * 60;// 7 day
    ...
    function withdrawToken(address to, uint amount) public onlyOwner {
        IERC20(kiloBank).transfer(to, amount);
    }
    ...
    fallback() external payable {
        ...
        if (block.timestamp >= priSaleEndTime) {
            IERC20(kiloBank).safeTransfer(communityReceiver, PriSaleLimit - priSoldTotal);
        } else {
            if (msg.value > 0) {
                partPriSale();
            }
    ...
    function partPriSale() internal {
        ...
        address refer = ITokenInterface(address(kiloBank)).getFather(msg.sender);
        uint referAward = 0;
        if (refer != address(0)) {
        ...
        if (release > 0) {
            IERC20(kiloBank).safeTransfer(msg.sender, release);
        }
        if (referAward > 0) {
            IERC20(kiloBank).safeTransfer(refer, referAward);
        }
    ...
    function getPriSalePrice() public view returns (uint) {
        if (block.timestamp < priSaleStartTime) {
            return 0;
        } else if (block.timestamp > priSaleStartTime && block.timestamp <= priSaleStartTime + Day7) {
            return StartPriSalePrice;
        } else if (block.timestamp > priSaleStartTime + Day7 && block.timestamp <= priSaleStartTime + Day7 * 2) {
    ...
    function burnPriSale(uint amount) public onlyOwner {
        ...
        ITokenInterface(address(kiloBank)).burn(amount);
    }
    ...
    function releasePriSale(address user) public override {
        ...
        if (priSalePrice != 0 && price >= 2 * priSalePrice) {
            ...
            if (info.lastReleaseTime != 0 && Day1 <= block.timestamp - info.lastReleaseTime &&
                ...
                kiloBank.safeTransfer(user, info.releaseAmtOnce);
            }
        }
    }
